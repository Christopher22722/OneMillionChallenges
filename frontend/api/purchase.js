...
module.exports = async (req, res) => {
  ...
  // ← NUEVO: normaliza los posibles nombres que puedan venir del cliente
  const imgUrl =
  ...
  try {
    ...
    for (const id of pixels) {
      console.log('purchase overlay:', { hasImg: !!imgUrl, color: overlay.color, pixels: pixels.length, orderId });
      const r = await client.query(
        `INSERT INTO pixels (id, color, img_url, link, order_id, buyer_email)
         VALUES ($1,$2,$3,$4,$5,$6)
         ON CONFLICT (id) DO NOTHING
         RETURNING id`,
          [id, overlay.color || null, imgUrl || null, link || null, orderId, buyerEmail || null]
      );
